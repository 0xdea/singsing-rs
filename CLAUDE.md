...
- `crates/zucchini` — binary crate, published as `zucchini-scanner` (the name `zucchini` was already taken on crates.io); still produces a `zucchini` binary. CLI front-end built on `singsing-rs` (`src/main.rs`).
