...
- `crates/zucchini-scanner` — binary crate (the name `zucchini` was already taken on crates.io); still produces a `zucchini` binary. CLI front-end built on `singsing-rs` (`src/main.rs`).
...
```sh
cargo run -p zucchini-scanner -- -i eth0 -h 192.168.2.10 -p 21-23,80,443
...
## `crates/zucchini-scanner/src/main.rs`
...
- `singsing-rs` and `zucchini-scanner` are published, versioned crates (crates.io); public API changes to `singsing-rs` are checked by `cargo-semver-checks` in CI.
- `crates/zucchini-scanner/Cargo.toml`'s package name is `zucchini-scanner`, but an explicit `[[bin]] name = "zucchini"` keeps the produced executable named `zucchini`. `main.rs` reads `env!("CARGO_BIN_NAME")` (not `CARGO_PKG_NAME`) for the banner/error-prefix text, and `Arguments`'s `#[command(name = "zucchini", ...)]` overrides clap's own default (which otherwise falls back to `CARGO_PKG_NAME` for the `Usage: ...` line) — both are required for the CLI to still say `zucchini` everywhere instead of `zucchini-scanner`.
...
- Sending to an address that the *scanning host's own* routing table resolves as a directed broadcast (`RTCF_BROADCAST`) fails with `EACCES` (`os error 13`), since the Linux kernel's `raw_sendmsg()` rejects it whenever `SO_BROADCAST` isn't set on the socket (`net/ipv4/raw.c`), and `pnet`'s raw socket never sets it. This is unrelated to the *target* CIDR's own network/broadcast exclusion in `parse_targets` — it depends on the scanning host's own interface/route configuration, not the target's, so it can surface on an address that looks like an ordinary host. Handy in practice: it's a real, easy way to trigger `ScanError::Incomplete`/`IncompleteScanError` end-to-end against a live scan instead of only via unit tests.
