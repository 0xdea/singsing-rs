...
- **Exit codes are meaningful**: clap usage errors (missing/invalid arguments) exit 2, `run()`-level `anyhow` errors (e.g. an unknown interface) exit 1 via `ExitCode::FAILURE`, and `--help` exits 0. On `ScanError::Incomplete`, `main` still prints whatever partial results were gathered (via the `IncompleteScanError` payload) before returning the error (exit 1). `singsing-rs`'s typed errors (`TargetsError`, `PortsError`, `ScanError`, ...) convert into `anyhow::Error` via `?` at every call site; the one place conversion runs the other way is `to_boxed_error`, which adapts a `write_*` helper's `anyhow::Result` into the `Box<dyn Error + Send + Sync>` the library's `on_result`/`on_progress` callbacks expect.
