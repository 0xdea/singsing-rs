...
- **Exit codes are meaningful**: clap usage errors (missing/invalid arguments) exit 2, `run()`-level `anyhow` errors (e.g. an unknown interface) exit 1 via `ExitCode::FAILURE`, and `--help` exits 0. On `IncompleteScanError`, `main` still prints whatever partial results were gathered before returning the error (exit 1).
